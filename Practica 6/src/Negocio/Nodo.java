...
public class Nodo<T> {
	...
	public Nodo<T> insertarNodo (Nodo<T> arbol, Comparable<T> elemento) {
		if (arbol == null) return new Nodo<T>((T) elemento);
		else if (elemento.compareTo(arbol.getElemento()) < 0) arbol.izq = insertarNodo(arbol.getIzq(), elemento);
		else if (elemento.compareTo(arbol.getElemento()) > 0) arbol.der = insertarNodo(arbol.getDer(), elemento);
		return arbol;
	}
}
