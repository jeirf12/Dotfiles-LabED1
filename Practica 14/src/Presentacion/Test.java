...
public class Test {
	...
	public static void main(String[] args) {
		llenarVerticesGrafo();
		if (!grafoLetras.getListaVertices().esVacia()) {
			grafoLetras.imprimirTodoGrafo();
		}
		if (!grafoNumerico.getListaVertices().esVacia()) {
			grafoNumerico.imprimirTodoGrafo();
		}
	}
	...
	public static void llenarAristas(String[] linea,int tipoGrafo) {
		if (tipoGrafo == 0) {
			grafoNumerico.agregarArista(Integer.parseInt(linea[0]), Integer.parseInt(linea[1]));
		} else if (tipoGrafo == 1) {
			grafoLetras.agregarArista(linea[0], linea[1]);
		}
	}
